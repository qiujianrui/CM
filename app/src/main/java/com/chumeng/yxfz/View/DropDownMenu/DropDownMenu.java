package com.chumeng.yxfz.View.DropDownMenu;
...
public class DropDownMenu extends LinearLayout {
    ...
    private void addTab(@NonNull List<String> tabTexts, int i) {
        final LinearLayout tabLinear=new LinearLayout(getContext());
        tabLinear.setOrientation(HORIZONTAL);
        tabLinear.setLayoutParams(new LayoutParams(0, ViewGroup.LayoutParams.WRAP_CONTENT, 1.0f));
...
//        tabText.setCompoundDrawablesWithIntrinsicBounds(null, null, getResources().getDrawable(menuUnselectedIcon), null);
        ...
        tabText.setPadding(dpTpPx(5), dpTpPx(12), dpTpPx(5), dpTpPx(12));
        tabLinear.addView(tabText);
        ImageView imageView=new ImageView(getContext());
        imageView.setLayoutParams(new LayoutParams(15,15));
        imageView.setImageDrawable(getResources().getDrawable(menuUnselectedIcon));
        tabLinear.addView(imageView);
        //添加点击事件
        tabLinear.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                switchMenu(tabLinear);
            }
        });
        tabMenuView.addView(tabLinear);
        //添加分割线
...
//            ((TextView) tabMenuView.getChildAt(current_tab_position)).setText(text);      QWERRRRRRRTYUIOP[]\
